  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-[var(--foreground)]">러닝 캘린더</h3>
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-[var(--foreground)] w-24 text-center">{year}.{month}월</span>
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {weekDays.map(d => (
          <div key={d} className={`text-center text-[10px] font-medium py-1 ${d === '일' ? 'text-red-400' : d === '토' ? 'text-blue-400' : 'text-[var(--muted)]'}`}>{d}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {days.map(day => (
          <button
            ...
            className={`aspect-square rounded-lg flex flex-col items-center justify-center text-xs font-medium transition-all hover:ring-2 hover:ring-[var(--accent)] ${getColor(day.count)} ${selectedDay === day.day ? 'ring-2 ring-[var(--accent)]' : ''}`}
          >
            <span className="text-[10px] leading-none">{day.day}</span>
            {day.count > 0 && <span className="text-[8px] font-bold leading-none mt-0.5">{day.count}명</span>}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 mt-3 pt-3 border-t border-[var(--card-border)] text-[9px] text-[var(--muted)]">
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-emerald-500" /> 50%+</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-emerald-300 dark:bg-emerald-600" /> 15%+</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-amber-200 dark:bg-amber-700" /> 1명+</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-slate-100 dark:bg-slate-800" /> 0명</span>
      </div>

      {/* 선택한 날의 상세 */}
      {selectedDayData && selectedDayData.count > 0 && (
        <div className="mt-3 pt-3 border-t border-[var(--card-border)]">
          <p className="text-xs font-semibold text-[var(--foreground)] mb-2">{month}/{selectedDayData.day} 러닝 ({selectedDayData.count}명)</p>
          <div className="flex flex-wrap gap-1">
            {selectedDayData.runners.sort((a, b) => b.distance - a.distance).map(r => (
              <span key={r.name} className="text-[11px] bg-[var(--card-border)] text-[var(--foreground)] px-2 py-1 rounded-lg">
