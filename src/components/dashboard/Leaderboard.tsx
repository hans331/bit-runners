    ...
    <div className="card">
            ...
            <div key={entry.member.id} className="group">
              <div className="flex items-center gap-2 md:gap-3">
                <span className="w-6 text-center text-xs">
                  {i < 3 ? medals[i] : <span className="text-[var(--muted)] font-mono">{i + 1}</span>}
                </span>
                <Link
                  href={`/member/${encodeURIComponent(entry.member.name)}`}
                  className="w-14 md:w-16 text-sm text-[var(--foreground)] hover:text-[var(--accent)] transition-colors truncate font-medium"
                >
                  {entry.member.name}
                </Link>
                <div className="flex-1 h-7 bg-slate-100 dark:bg-slate-800/50 rounded-full overflow-hidden relative">
                  <div
                    className={`h-full rounded-full transition-all duration-700 flex items-center justify-end pr-2 ${
                      isFinisher
                        ? 'bg-gradient-to-r from-emerald-400 to-emerald-500 dark:from-emerald-600 dark:to-emerald-400'
                ...
                {barWidth <= 25 && (
                  <span className="w-16 text-right text-xs font-mono text-[var(--muted)]">
                    {entry.distance.toFixed(1)}km
                  </span>
                )}
      ...
      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-[var(--card-border)] text-[10px] text-[var(--muted)]">
        <span className="flex items-center gap-1">
          <span className="w-3 h-2 rounded-sm bg-emerald-500" /> 달성
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-2 rounded-sm bg-blue-500" /> 진행중
        </span>
        <span className="flex items-center gap-1">
          <span className="w-0.5 h-3 bg-amber-500" /> 목표
        </span>
      </div>
    </div>
  );
