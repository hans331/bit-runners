      ...
      <div className="h-56 md:h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 20, right: 10, left: -10, bottom: 5 }}>
            ...
            <YAxis tick={{ fill: getTextColor(isDark), fontSize: 11 }} axisLine={{ stroke: getAxisColor(isDark) }} />
            ...
            <Area type="monotone" dataKey="total" stroke="#3b82f6" strokeWidth={2.5} fill="url(#colorTotal)" dot={{ r: 4, fill: '#3b82f6', strokeWidth: 0 }} activeDot={{ r: 6, fill: '#60a5fa' }}>
              <LabelList dataKey="total" position="top" formatter={(v: unknown) => `${Number(v).toFixed(0)}`} style={{ fill: getTextColor(isDark), fontSize: 10, fontWeight: 600 }} />
            </Area>
          </AreaChart>
        </ResponsiveContainer>
      </div>
