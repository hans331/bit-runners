      ...
      {showAddForm && (
        <form onSubmit={handleAddMember} className="card space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div><label className="block text-xs font-medium text-[var(--foreground)] mb-1">이름 *</label>
            ...
            <div><label className="block text-xs font-medium text-[var(--foreground)] mb-1">합류 장소</label>
            ...
            <div><label className="block text-xs font-medium text-[var(--foreground)] mb-1">합류일</label>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-[var(--muted)] text-left border-b border-[var(--card-border)] bg-[var(--sidebar-bg)]">
              <th className="py-3 px-4 font-medium text-xs">번호</th><th className="py-3 px-4 font-medium text-xs">이름</th>
              <th className="py-3 px-4 font-medium text-xs hidden sm:table-cell">합류</th><th className="py-3 px-4 font-medium text-xs text-right">통산</th>
              <th className="py-3 px-4 font-medium text-xs text-center hidden sm:table-cell">활동</th><th className="py-3 px-4 font-medium text-xs text-center hidden md:table-cell">최근</th>
              <th className="py-3 px-4 font-medium text-xs text-center">상태</th><th className="py-3 px-4 font-medium text-xs text-center">관리</th>
            </tr></thead>
            <tbody>
              {filtered.map(m => (
                <tr key={m.id} className={`border-b border-[var(--card-border)] last:border-0 transition-colors ${m.status === 'dormant' ? 'opacity-60' : 'hover:bg-[var(--card-border)]/50'}`}>
                  ...
                  <td className="py-3 px-4 text-xs text-[var(--muted)] hidden sm:table-cell">{m.join_location || '-'}{m.join_date && <span className="block text-[10px]">{m.join_date}</span>}</td>
                  <td className="py-3 px-4 text-right font-mono text-xs font-semibold">{m.totalDistance.toFixed(0)}km</td>
                  ...
                  <td className="py-3 px-4 text-center">
                    <span className={`inline-flex text-[10px] font-semibold px-2 py-0.5 rounded-full ${m.status === 'active' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400'}`}>{m.status === 'active' ? '활동' : '휴면'}</span>
                  </td>
                  <td className="py-3 px-4 text-center">
                    <button onClick={() => handleStatusToggle(m.id, m.status)} disabled={loading}
                      className={`text-[10px] font-medium px-2.5 py-1 rounded-lg transition-colors ${m.status === 'active' ? 'text-amber-600 dark:text-amber-400 hover:bg-amber-500/10' : 'text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/10'}`}>
