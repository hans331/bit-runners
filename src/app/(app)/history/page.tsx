    ...
    <div className="p-4 max-w-lg mx-auto space-y-4 pb-8">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="card p-4 relative overflow-hidden">
          ...
          <p className="text-[10px] text-[var(--muted)] mt-1">{clubSummary ? `${clubSummary.activeMembers}명 활동` : ''}</p>
        </div>
        <div className="card p-4 relative overflow-hidden">
        ...
        <div className="card p-4 relative overflow-hidden">
          ...
          <p className="text-[10px] text-[var(--muted)] mt-1">D-{clubSummary?.daysRemaining ?? '–'}</p>
        </div>
        <div className="card p-4 relative overflow-hidden">
          ...
          <p className="text-[10px] text-[var(--muted)] mt-1">{clubSummary ? `전체 ${clubSummary.totalMembers}명` : ''}</p>
        </div>
      </div>
            ...
            {sortedByDistance.map((m, i) => {
              ...
              return (
                <div key={m.user_id} className="flex items-center gap-2">
                  ...
                  <span className="w-14 text-xs font-medium text-[var(--foreground)] truncate flex-shrink-0">{m.display_name}</span>
                  ...
                  {m.progress >= 100 && <span className="text-xs">✅</span>}
                </div>
              );
            })}
          </div>
            ...
            {membersWithGoal.map(m => (
              <div key={m.user_id}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-medium text-[var(--foreground)]">{m.display_name}</span>
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-xs mb-2">
          {['일','월','화','수','목','금','토'].map(d => (
            <span key={d} className="text-[var(--muted)] py-1 text-[10px]">{d}</span>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div
                key={day}
                className={`aspect-square flex flex-col items-center justify-center rounded-lg text-[10px] font-medium relative ${bgClass}`}
              >
                <span>{day}</span>
                {clubCount > 0 && (
                  <span className="text-[8px] opacity-80">{clubCount}명</span>
                )}
              </div>
            );
          })}
        ...
        {/* 범례 */}
        <div className="flex items-center gap-3 mt-3 text-[10px] text-[var(--muted)] justify-center">
            ...
            {runCounts.map((r, i) => {
              ...
              return (
                <div key={r.user_id} className="flex items-center gap-2">
                  <span className="w-14 text-xs font-medium text-[var(--foreground)] truncate flex-shrink-0">{r.display_name}</span>
                  <div className="flex-1 h-5 bg-[var(--card-border)] rounded-full overflow-hidden">
                    <div
                  ...
                  <span className="text-xs font-semibold text-[var(--foreground)] w-8 text-right flex-shrink-0">{r.run_count}회</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="space-y-2.5">
            {cumRanking.map((r, i) => (
              <div key={r.user_id} className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-[var(--foreground)] truncate flex-shrink-0 w-14">{r.display_name}</span>
                <div className="flex-1 h-5 bg-[var(--card-border)] rounded-full overflow-hidden">
                  <div
          ...
          <div className="space-y-5">
            {hallOfFame.map(entry => (
              <div key={entry.category}>
                ...
                <p className="text-[10px] text-[var(--muted)] mb-2">{entry.description}</p>
                  ...
                  {entry.winners.map((w, i) => (
                    <div key={w.user_id} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium text-[var(--foreground)]">{w.display_name}</span>
                      </div>
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-3 gap-4 text-center mb-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--muted)]">러닝</p>
          </div>
          <div>
